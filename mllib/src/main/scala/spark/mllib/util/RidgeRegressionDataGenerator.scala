...
object RidgeRegressionDataGenerator {
  ...
  def generateRidgeRDD(
    ...
    nfeatures: Int,
    eps: Double,
    nparts: Int = 2) : RDD[(Double, DenseVector)] = {
    ...
    val data: RDD[(Double, DenseVector)] = sc.parallelize(0 until nparts, nparts).flatMap { p =>
      ...
      Iterator.tabulate(examplesInPartition) { i =>
        (yObs.get(i, 0), new DenseVector(X.getRow(i).toArray))
      }
    }
    data
  }
