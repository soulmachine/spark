 ...
 */
final object Arithmetic {
  // for method STIRLING_CORRECTION(...)
  private val STIRLING_CORRECTION = Array(
    0.0,
  ...
  // log(k!) for k = 0, ..., 29
  private val LOG_FACTORIALS = Array(
  ...
  // k! for k = 0, ..., 20
  private val LONG_FACTORIALS = Array(
    1L,
    1L,
    2L,
    6L,
  ...
  // k! for k = 21, ..., 170
  private val DOUBLE_FACTORIALS = Array(
   ...
   * @return the binomial coefficient.
   */
  def binomial(n: Long, _k: Long): Double = {
    var k = _k
    if (k < 0) {
      return 0
    }
    ...
    if (n > k) {
      val max = LONG_FACTORIALS.length + DOUBLE_FACTORIALS.length;
      if (n < max) { // if (n! < inf && k! < inf)
        ...
        if (nk != Double.PositiveInfinity) { // no numeric overflow?
   ...
   */
  private def factorial(k: Int): Double = {
    if (k < 0) {
      throw new IllegalArgumentException();
    }

    val length1 = LONG_FACTORIALS.length
    if (k < length1) {
      return LONG_FACTORIALS(k)
    }

    val length2 = DOUBLE_FACTORIALS.length
    if (k < length1 + length2) {
      DOUBLE_FACTORIALS(k - length1)
    } else {
  ...
  def stirlingCorrection(k: Int): Double = {
    ...
    if (k > 30) {
      val r = 1.0 / k
      val rr = r * r
      val C7 = -5.95238095238095238e-04     //  -1/1680
      val C5 = 7.93650793650793651e-04     //  +1/1260
      val C3 = -2.77777777777777778e-03     //  -1/360
      val C1 = 8.33333333333333333e-02     //  +1/12
